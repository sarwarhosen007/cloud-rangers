        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {CASE_STUDIES.map((c) => (
            <Link
              ...
              to="/work/$slug"
              params={{ slug: c.slug }}
              className="group flex flex-col overflow-hidden rounded-xl border border-border bg-surface/60 p-6 transition hover:border-primary/50 hover:bg-surface"
            >
              <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-muted-foreground">
                {c.industry}
              </p>
              <h3 className="mt-2 text-xl font-semibold tracking-tight">{c.client}</h3>
